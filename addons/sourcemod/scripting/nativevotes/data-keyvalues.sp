...
KeyValues Data_CreateVote(NativeVotesType voteType, MenuAction actions)
{
...
{
	ArrayList infoArray = view_as<ArrayList>(KvGetNum(vote, INFO, view_as<int>(INVALID_HANDLE)));
	ArrayList displayArray = view_as<ArrayList>(KvGetNum(vote, DISPLAY, view_as<int>(INVALID_HANDLE)));
...
void Data_CloseVote(KeyValues vote)
{
	...
	Handle handler = view_as<Handle>(vote.GetNum("handler_callback", view_as<int>(INVALID_HANDLE)));
	if (handler != null)
	{
		delete handler;
	}
	
	Handle voteResults = view_as<Handle>(vote.GetNum("results_callback", view_as<int>(INVALID_HANDLE)));
	if (voteResults != null)
	{
		delete voteResults;
	}
